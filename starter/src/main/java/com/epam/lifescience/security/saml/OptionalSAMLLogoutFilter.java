...
public class OptionalSAMLLogoutFilter extends LogoutFilter {
     ...
     */
    public OptionalSAMLLogoutFilter(final String successUrl, final LogoutHandler[] localHandler,
     ...
     * @param globalHandlers       handlers to be invoked when global logout is selected
     */
    public OptionalSAMLLogoutFilter(final LogoutSuccessHandler logoutSuccessHandler,
                                    final LogoutHandler[] localHandler, final LogoutHandler[] globalHandlers) {
    ...
    public void processLogout(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (requiresLogout(request, response)) {
            ...
            if (auth != null && isGlobalLogout(request, auth)) {
                ...
                Arrays.stream(globalHandlers).forEach((handler) -> handler.logout(request, response, auth));

                // Notify session participants using SAML Single Logout profile
                final SAMLCredential credential = (SAMLCredential) auth.getCredentials();
    ...
    protected boolean isGlobalLogout(final HttpServletRequest request, final Authentication auth) {
        final String localLogout = request.getParameter(LOGOUT_PARAMETER);
     ...
     */
    @Autowired
    public void setSamlLogger(final SAMLLogger samlLogger) {
     ...
     */
    @Autowired
    public void setProfile(final SingleLogoutProfile profile) {
     ...
     */
    @Autowired
    public void setContextProvider(final SAMLContextProvider contextProvider) {
